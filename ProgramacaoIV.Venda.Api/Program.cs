...
{
    var cliente = await context.Clientes.FirstOrDefaultAsync(x => x.Id == request.IdCliente && x.IsAtivo);
    var vendedor = await context.Vendedores.FirstOrDefaultAsync(x => x.Id == request.VendedoresId && x.IsAtivo);

    if (cliente is null || vendedor is null)
        return Results.BadRequest("Cliente ou vendedor inválido ou inativo.");
...
app.MapPost("/vendedores", async ([FromBody] Vendedores vendedores, VendaContext context) =>
{
    context.Vendedores.Add(vendedores);
    await context.SaveChangesAsync();
    return Results.Created($"/vendedores/{vendedores.Id}", vendedores);
});

app.MapGet("/vendedores", async (VendaContext context) =>
{
    var vendedores = await context.Vendedores.Where(x => x.IsAtivo).ToListAsync();
    return Results.Ok(vendedores);
});

app.MapGet("/vendedores/{id}", async (string id, VendaContext context) =>
{
    var vendedores = await context.Vendedores.Where(x => x.Id == int.Parse(id) && x.IsAtivo).FirstOrDefaultAsync();
    return vendedores is null ? Results.NotFound() : Results.Ok(vendedores);
});

app.MapPut("/vendedores/{id}", async (string id, [FromBody] Vendedores input, VendaContext context) =>
{
    var vendedores = await context.Vendedores.FirstOrDefaultAsync(x => x.Id == int.Parse(id) && x.IsAtivo);
    if (vendedores is null) return Results.NotFound();

    vendedores.Atualizar(input.Nome, input.Email);
    context.Vendedores.Update(vendedores);
    await context.SaveChangesAsync();
